...
void CRpgRocket :: Spawn( void )
{
	...
	pev->velocity = gpGlobals->v_forward * 400;  // Increased from 250 to 400 (faster rocket)
	pev->gravity = 0.3;  // Reduced from 0.5 to 0.3 (less drop)

	pev->nextthink = gpGlobals->time + 0.4;

	pev->dmg = gSkillData.plrDmgRPG * 1.5;  // 50% more damage
}
...
int CRpg::GetItemInfo(ItemInfo *p)
{
	...
	p->pszAmmo1 = "rockets";
	p->iMaxAmmo1 = 10;  // Increased from 5 to 10 rockets
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
...
class CRpgAmmo : public CBasePlayerAmmo
{
	...
	if ( g_pGameRules->IsMultiplayer() )
#endif
		{
			...
			iGive = 2;  // Give 2 rockets in multiplayer
		}
		else
		{
			iGive = 2;  // Give 2 rockets in singleplayer (increased from 1)
		}

		if (pOther->GiveAmmo( iGive, "rockets", 10 ) != -1)  // Max 10 rockets
		{
