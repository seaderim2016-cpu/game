...
int CEgon::GetItemInfo(ItemInfo *p)
{
	...
	p->pszAmmo1 = "762";  // Changed from uranium to 7.62mm NATO
	p->iMaxAmmo1 = 400;    // Changed from URANIUM_MAX_CARRY to 400 rounds
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = 100;     // Changed from WEAPON_NOCLIP to 100 round belt
	p->iSlot = 3;
	p->iPosition = 2;
...
{
	// M60 Machine Gun - Automatic fire
	// don't fire underwater
	if ( m_pPlayer->pev->waterlevel == 3 )
	{
		PlayEmptySound( );
	...
	// Fire bullets with medium spread
	Vector vecSpread = VECTOR_CONE_5DEGREES; // Medium spread for M60
	// Using 357 bullet type for 7.62mm (higher damage)
	m_pPlayer->FireBullets( 1, vecSrc, vecAiming, vecSpread, 8192, BULLET_PLAYER_357, 2 );
	...
	PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usEgonFire, 0.0, (float *)&g_vecZero, (float *)&g_vecZero, 0.0, 0.0, 0, 0, 0, 0 );

	// Player animation
	m_pPlayer->SetAnimation( PLAYER_ATTACK1 );

	// Recoil (stronger than M249)
	m_pPlayer->pev->punchangle.x -= UTIL_SharedRandomFloat( m_pPlayer->random_seed, 1.5, 2.0 );

	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.2;
	m_flNextPrimaryAttack = GetNextAttackDelay(0.12); // 500 RPM (0.12 sec between shots)
}
...
class CEgonAmmo : public CBasePlayerAmmo
{
	...
	{ 
		// M60 uses 7.62mm ammo
		if (pOther->GiveAmmo( 100, "762", 400 ) != -1)
		{
