...
int CGauss::GetItemInfo(ItemInfo *p)
{
	...
	p->pszAmmo1 = "556";  // Changed from uranium to 5.56mm NATO
	p->iMaxAmmo1 = 600;    // Changed from URANIUM_MAX_CARRY to 600 rounds
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = 200;     // Changed from WEAPON_NOCLIP to 200 round box
	p->iSlot = 3;
	p->iPosition = 1;
...
void CGauss::PrimaryAttack()
{
	...
	// Fire bullets with spread (less accurate when sustained fire)
	Vector vecSpread = VECTOR_CONE_6DEGREES; // Medium spread for LMG
	// Using MP5 bullet type for 5.56mm (similar damage)
	m_pPlayer->FireBullets( 1, vecSrc, vecAiming, vecSpread, 8192, BULLET_PLAYER_MP5, 2 );
	...
	PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usGaussFire, 0.0, (float *)&g_vecZero, (float *)&g_vecZero, 0.0, 0.0, 0, 0, 0, 0 );

	// Player animation
	m_pPlayer->SetAnimation( PLAYER_ATTACK1 );

	// Recoil
	m_pPlayer->pev->punchangle.x -= UTIL_SharedRandomFloat( m_pPlayer->random_seed, 1.0, 1.5 );

	m_fInAttack = 0;
	m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.2;
	m_flNextPrimaryAttack = GetNextAttackDelay(0.1); // 600 RPM (0.1 sec between shots)
}

void CGauss::SecondaryAttack()
{
	// M249 SAW - No secondary attack (removed charging mechanism)
	// Just use primary attack
	return;
}
...
class CGaussAmmo : public CBasePlayerAmmo
{
	...
	{ 
		// M249 SAW uses 5.56mm ammo
		if (pOther->GiveAmmo( 200, "556", 600 ) != -1)
		{
