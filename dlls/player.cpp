...
if (m_iCharacterType == 1) // Blues
	{
		// Blue Glow Shell effect
		pev->renderfx = kRenderFxGlowShell;
		pev->renderamt = 100; // Glow thickness
		pev->rendercolor = Vector(0, 0, 255); // Blue
	}
	else if (m_iCharacterType == 2) // Thief
	{
		if (pev->flags & FL_DUCKING)
		{
			// Stealth transparency
			pev->rendermode = kRenderTransTexture;
			pev->renderamt = 50; // Semi-invisible
		}
		else
		{
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// Character specific damage reduction
	if (m_iCharacterType == 1) // Blues Shield
	{
		flDamage *= 0.7; // 30% reduction
	}
...
void CBasePlayer::StartObserver( Vector vecPosition, Vector vecViewAngle )
{
	...
	// Setup flags
	m_iHideHUD = 0;
	m_iCharacterType = 0; // Default

	char *pModelName = gEngfuncs.pfnInfoKeyValue( gEngfuncs.pfnGetInfoKeyBuffer( edict() ), "model" );
	if ( pModelName )
	{
		if ( strstr( pModelName, "blues" ) ) m_iCharacterType = 1;
		else if ( strstr( pModelName, "thief" ) ) m_iCharacterType = 2;
	}

	// For demonstration, if no specific model, we can cycle if needed, 
	// but let's stick to model name for now as it's the standard way.
...
void CBasePlayer::PreThink(void)
{
	...
	// Thief Wall Jump logic
	if (m_iCharacterType == 2 && !(pev->flags & FL_ONGROUND) && m_flWallJumpTime < gpGlobals->time)
	{
		UTIL_MakeVectors(pev->angles);
		Vector vecForward = gpGlobals->v_forward;
		TraceResult tr;
		
		// Check for wall behind or to the sides
		UTIL_TraceLine(pev->origin, pev->origin - vecForward * 32, ignore_monsters, ENT(pev), &tr);
		if (tr.flFraction < 1.0)
		{
			pev->velocity = pev->velocity + vecForward * 400 + Vector(0, 0, 300);
			m_flWallJumpTime = gpGlobals->time + 0.5;
