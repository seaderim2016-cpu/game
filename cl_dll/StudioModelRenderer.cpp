...
void CStudioModelRenderer::Init(void) {
  ...
  if (m_pCvarHiModels)
    m_pCvarHiModels->value = 1.0f; // Force high quality models
  ...
  m_pbonetransform =
      (float (*)[MAXSTUDIOBONES][3][4])IEngineStudio.StudioGetBoneTransform();
  m_plighttransform =
      (float (*)[MAXSTUDIOBONES][3][4])IEngineStudio.StudioGetLightTransform();
...
====================
*/
void CStudioModelRenderer::StudioCalcBoneAdj(float dadt, float *adj,
  ...
  pbonecontroller =
      (mstudiobonecontroller_t *)((byte *)m_pStudioHeader +
    ...
    if (i <= 3) {
      ...
      if (pbonecontroller[j].type & STUDIO_RLOOP) {
        if (abs(pcontroller1[i] - pcontroller2[i]) > 128) {
          ...
          value = ((a * dadt) + (b * (1 - dadt)) - 128) * (360.0 / 256.0) +
                  pbonecontroller[j].start;
        } else {
          value =
              ((pcontroller1[i] * dadt + (pcontroller2[i]) * (1.0 - dadt))) *
                  (360.0 / 256.0) +
              pbonecontroller[j].start;
        }
      } else {
        value =
            (pcontroller1[i] * dadt + pcontroller2[i] * (1.0 - dadt)) / 255.0;
        if (value < 0)
          value = 0;
        if (value > 1.0)
          value = 1.0;
        value = (1.0 - value) * pbonecontroller[j].start +
                value * pbonecontroller[j].end;
      }
      // Con_DPrintf( "%d %d %f : %f\n",
      // m_pCurrentEntity->curstate.controller[j],
      // m_pCurrentEntity->latched.prevcontroller[j], value, dadt );
    } else {
...
====================
*/
void CStudioModelRenderer::StudioCalcBoneQuaterion(int frame, float s,
                                                   mstudiobone_t *pbone,
                                                   mstudioanim_t *panim,
                                                   float *adj, float *q) {
  int j, k;
...
====================
*/
void CStudioModelRenderer::StudioCalcBonePosition(int frame, float s,
                                                  mstudiobone_t *pbone,
                                                  mstudioanim_t *panim,
                                                  float *adj, float *pos) {
  int j, k;
  ...
  for (j = 0; j < 3; j++) {
    ...
    if (panim->offset[j] != 0) {
      ...
      /*
      if (i == 0 && j == 0)
              Con_DPrintf("%d  %d:%d  %f\n", frame, panimvalue->num.valid,
      panimvalue->num.total, s );
      */
      ...
      if (panimvalue->num.valid > k) {
        ...
        if (panimvalue->num.valid > k + 1) {
          pos[j] += (panimvalue[k + 1].value * (1.0 - s) +
                     s * panimvalue[k + 2].value) *
                    pbone->scale[j];
        } else {
          pos[j] += panimvalue[k + 1].value * pbone->scale[j];
        }
      } else {
        // are we at the end of the repeating values section and there's another
        // section with data?
        if (panimvalue->num.total <= k + 1) {
...
====================
*/
void CStudioModelRenderer::StudioSlerpBones(vec4_t q1[], float pos1[][3],
                                            vec4_t q2[], float pos2[][3],
                                            float s) {
  int i;
  vec4_t q3;
  float s1;
...
====================
*/
mstudioanim_t *CStudioModelRenderer::StudioGetAnim(model_t *m_pSubModel,
                                                   mstudioseqdesc_t *pseqdesc) {
  mstudioseqgroup_t *pseqgroup;
  cache_user_t *paSequences;

  pseqgroup = (mstudioseqgroup_t *)((byte *)m_pStudioHeader +
                                    m_pStudioHeader->seqgroupindex) +
              pseqdesc->seqgroup;
  ...
  if (paSequences == NULL) {
    paSequences = (cache_user_t *)IEngineStudio.Mem_Calloc(
        16, sizeof(cache_user_t)); // UNDONE: leak!
    m_pSubModel->submodels = (dmodel_t *)paSequences;
  }

  if (!IEngineStudio.Cache_Check(
          (struct cache_user_s *)&(paSequences[pseqdesc->seqgroup]))) {
    gEngfuncs.Con_DPrintf("loading %s\n", pseqgroup->name);
    IEngineStudio.LoadCacheFile(
        pseqgroup->name,
        (struct cache_user_s *)&paSequences[pseqdesc->seqgroup]);
  }
  return (mstudioanim_t *)((byte *)paSequences[pseqdesc->seqgroup].data +
                           pseqdesc->animindex);
}
...
====================
*/
void CStudioModelRenderer::StudioPlayerBlend(mstudioseqdesc_t *pseqdesc,
  ...
  if (*pBlend < pseqdesc->blendstart[0]) {
    ...
    *pBlend = 255;
    ...
    if (pseqdesc->blendend[0] - pseqdesc->blendstart[0] < 0.1) // catch qc error
      ...
      *pBlend = 255 * (*pBlend - pseqdesc->blendstart[0]) /
                (pseqdesc->blendend[0] - pseqdesc->blendstart[0]);
    *pPitch = 0;
  }
}
...
void CStudioModelRenderer::StudioSetUpTransform(int trivial_accept) {
  ...
  // m_pCurrentEntity->index); Con_DPrintf("movetype %d %d\n",
  ...
  if (m_pCurrentEntity->curstate.movetype == MOVETYPE_STEP) {
    ...
    // NOTE:  Because we need to interpolate multiplayer characters, the
    // interpolation time limit
    //  was increased to 1.0 s., which is 2x the max lag we are accounting for.

    if ((m_clTime < m_pCurrentEntity->curstate.animtime + 1.0f) &&
        (m_pCurrentEntity->curstate.animtime !=
         m_pCurrentEntity->latched.prevanimtime)) {
      f = (m_clTime - m_pCurrentEntity->curstate.animtime) /
          (m_pCurrentEntity->curstate.animtime -
           m_pCurrentEntity->latched.prevanimtime);
    ...
    if (m_fDoInterp) {
      // ugly hack to interpolate angle, position. current is reached 0.1
    ...
    for (i = 0; i < 3; i++) {
      modelpos[i] += (m_pCurrentEntity->origin[i] -
                      m_pCurrentEntity->latched.prevorigin[i]) *
                     f;
    }

    // NOTE:  Because multiplayer lag can be relatively large, we don't want to
    // cap
    //  f at 1.5 anymore.
    // if (f > -1.0 && f < 1.5) {}

    //			Con_DPrintf("%.0f
    //%.0f\n",m_pCurrentEntity->msg_angles[0][YAW],
    //m_pCurrentEntity->msg_angles[1][YAW] );
    for (i = 0; i < 3; i++) {
      float ang1, ang2;
  ...
  if (!IEngineStudio.IsHardware()) {
    ...
    // do the scaling up of x and y to screen coordinates as part of the
    // transform for the unclipped case (it would mess up clipping in the
    // clipped case). Also scale down z, so 1/z is scaled 31 bits for free, and
    // scale down x and y correspondingly so the projected x and y come out
    // right
    // FIXME: make this work for clipped case too?
    if (trivial_accept) {
      for (i = 0; i < 4; i++) {
        (*m_paliastransform)[0][i] *=
            m_fSoftwareXScale * (1.0 / (ZISCALE * 0x10000));
        (*m_paliastransform)[1][i] *=
            m_fSoftwareYScale * (1.0 / (ZISCALE * 0x10000));
...
float CStudioModelRenderer::StudioEstimateInterpolant(void) {
  ...
  if (m_fDoInterp && (m_pCurrentEntity->curstate.animtime >=
                      m_pCurrentEntity->latched.prevanimtime + 0.01)) {
...
====================
*/
void CStudioModelRenderer::StudioCalcRotations(float pos[][3], vec4_t *q,
                                               mstudioseqdesc_t *pseqdesc,
  ...
  // m_pCurrentEntity->curstate.sequence, m_clTime, m_pCurrentEntity->animtime,
  // m_pCurrentEntity->frame, f, frame );

  // Con_DPrintf( "%f %f %f\n", m_pCurrentEntity->angles[ROLL],
  // m_pCurrentEntity->angles[PITCH], m_pCurrentEntity->angles[YAW] );
  ...
  pbone =
      (mstudiobone_t *)((byte *)m_pStudioHeader + m_pStudioHeader->boneindex);

  StudioCalcBoneAdj(dadt, adj, m_pCurrentEntity->curstate.controller,
                    m_pCurrentEntity->latched.prevcontroller,
                    m_pCurrentEntity->mouth.mouthopen);
  ...
  for (i = 0; i < m_pStudioHeader->numbones; i++, pbone++, panim++) {
    ...
    // if (0 && i == 0)
    //	Con_DPrintf("%d %d %d %d\n", m_pCurrentEntity->curstate.sequence, frame,
    //j, k );
  }
  ...
  s = 0 * ((1.0 - (f - (int)(f))) / (pseqdesc->numframes)) *
...
====================
*/
void CStudioModelRenderer::StudioFxTransform(cl_entity_t *ent,
                                             float transform[3][4]) {
...
void CStudioModelRenderer::StudioSetupBones(void) {
  ...
  /*	if ( m_pPlayerInfo )
          {
                  ...
                  if ( m_nPlayerGaitSequences[ playerNum ] !=
  m_pPlayerInfo->gaitsequence )
                  {
          //		m_pPlayerInfo->gaitframe = 0.0;
                          gEngfuncs.Con_Printf( "Setting gaitframe to 0\n" );
                  }

                  m_nPlayerGaitSequences[ playerNum ] =
  m_pPlayerInfo->gaitsequence;
  //		gEngfuncs.Con_Printf( "index: %d     gaitsequence:
  ...
  */
  ...
  if (pseqdesc->numblends > 1) {
    ...
    dadt = StudioEstimateInterpolant();
    s = (m_pCurrentEntity->curstate.blending[0] * dadt +
         m_pCurrentEntity->latched.prevblending[0] * (1.0 - dadt)) /
        255.0;
    ...
    if (pseqdesc->numblends == 4) {
      ...
      s = (m_pCurrentEntity->curstate.blending[1] * dadt +
           m_pCurrentEntity->latched.prevblending[1] * (1.0 - dadt)) /
          255.0;
  ...
  if (m_fDoInterp && m_pCurrentEntity->latched.sequencetime &&
    ...
    pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader +
                                    m_pStudioHeader->seqindex) +
               m_pCurrentEntity->latched.prevsequence;
    panim = StudioGetAnim(m_pRenderModel, pseqdesc);
    // clip prevframe
    StudioCalcRotations(pos1b, q1b, pseqdesc, panim,
                        m_pCurrentEntity->latched.prevframe);
    ...
    if (pseqdesc->numblends > 1) {
      ...
      if (pseqdesc->numblends == 4) {
        panim += m_pStudioHeader->numbones;
        StudioCalcRotations(pos3, q3, pseqdesc, panim,
                            m_pCurrentEntity->latched.prevframe);

        panim += m_pStudioHeader->numbones;
        StudioCalcRotations(pos4, q4, pseqdesc, panim,
                            m_pCurrentEntity->latched.prevframe);
  ...
  for (i = 0; i < m_pStudioHeader->numbones; i++) {
    ...
    if (pbones[i].parent == -1) {
      if (IEngineStudio.IsHardware()) {
        ConcatTransforms((*m_protationmatrix), bonematrix,
                         (*m_pbonetransform)[i]);
        ...
        ConcatTransforms((*m_paliastransform), bonematrix,
                         (*m_pbonetransform)[i]);
        ConcatTransforms((*m_protationmatrix), bonematrix,
                         (*m_plighttransform)[i]);
      }
      ...
      ConcatTransforms((*m_pbonetransform)[pbones[i].parent], bonematrix,
                       (*m_pbonetransform)[i]);
      ConcatTransforms((*m_plighttransform)[pbones[i].parent], bonematrix,
                       (*m_plighttransform)[i]);
    }
  }
}
...
#define IS_FIRSTPERSON_SPEC                                                    \
  (g_iUser1 == OBS_IN_EYE ||                                                   \
   (g_iUser1 && (gHUD.m_Spectator.m_pip->value == INSET_IN_EYE)))
...
int CStudioModelRenderer::StudioDrawModel(int flags) {
  ...
  if (m_pCurrentEntity->curstate.renderfx == kRenderFxDeadPlayer) {
    ...
    if (m_pCurrentEntity->curstate.renderamt <= 0 ||
        m_pCurrentEntity->curstate.renderamt > gEngfuncs.GetMaxClients())
      return 0;

    // get copy of player
    deadplayer = *(IEngineStudio.GetPlayerState(
        m_pCurrentEntity->curstate.renderamt -
        1)); // cl.frames[cl.parsecount &
             // CL_UPDATE_MASK].playerstate[m_pCurrentEntity->curstate.renderamt-1];
  ...
  if (flags & STUDIO_RENDER) {
    ...
    // team 2
    else if ((m_nTopColor < 260) &&
             ((m_nTopColor > 240) || (m_nTopColor == 5))) {
    ...
    // is this our view model and should it be glowing? we also fix a remap
    // colors problem if we're spectating in first-person mode
    if (m_pCurrentEntity == gEngfuncs.GetViewModel()) {
      ...
      if (IS_FIRSTPERSON_SPEC) {
        ...
        if (pTarget) {
          ...
          // the same conditions need to be met (this is the view model and
          // we're in first-person spectator mode)
          m_nTopColor =
              GetRemapColor(g_PlayerExtraInfo[pTarget->index].teamnumber, true);
          m_nBottomColor = GetRemapColor(
              g_PlayerExtraInfo[pTarget->index].teamnumber, false);
        }
      }
      ...
      if (pTarget && pTarget->curstate.renderfx == kRenderFxGlowShell) {
        ...
        m_pCurrentEntity->curstate.rendercolor.r =
            pTarget->curstate.rendercolor.r;
        m_pCurrentEntity->curstate.rendercolor.g =
            pTarget->curstate.rendercolor.g;
        m_pCurrentEntity->curstate.rendercolor.b =
            pTarget->curstate.rendercolor.b;
      } else {
...
void CStudioModelRenderer::StudioEstimateGait(entity_state_t *pplayer) {
  ...
  if (m_fGaitEstimation) {
    VectorSubtract(m_pCurrentEntity->origin, m_pPlayerInfo->prevgaitorigin,
                   est_velocity);
  ...
  if (est_velocity[1] == 0 && est_velocity[0] == 0) {
    ...
    m_pPlayerInfo->gaityaw =
        m_pPlayerInfo->gaityaw - (int)(m_pPlayerInfo->gaityaw / 360) * 360;

    m_flGaitMovement = 0;
  } else {
    m_pPlayerInfo->gaityaw =
        (atan2(est_velocity[1], est_velocity[0]) * 180 / M_PI);
...
void CStudioModelRenderer::StudioProcessGait(entity_state_t *pplayer) {
  ...
  m_pCurrentEntity->latched.prevblending[0] =
      m_pCurrentEntity->curstate.blending[0];
  m_pCurrentEntity->latched.prevseqblending[0] =
      m_pCurrentEntity->curstate.blending[0];

  // Con_DPrintf("%f %d\n", m_pCurrentEntity->angles[PITCH],
  ...
  // adjust torso
  m_pCurrentEntity->curstate.controller[0] =
      ((flYaw / 4.0) + 30) / (60.0 / 255.0);
  m_pCurrentEntity->curstate.controller[1] =
      ((flYaw / 4.0) + 30) / (60.0 / 255.0);
  m_pCurrentEntity->curstate.controller[2] =
      ((flYaw / 4.0) + 30) / (60.0 / 255.0);
  m_pCurrentEntity->curstate.controller[3] =
      ((flYaw / 4.0) + 30) / (60.0 / 255.0);
  m_pCurrentEntity->latched.prevcontroller[0] =
      m_pCurrentEntity->curstate.controller[0];
  m_pCurrentEntity->latched.prevcontroller[1] =
      m_pCurrentEntity->curstate.controller[1];
  m_pCurrentEntity->latched.prevcontroller[2] =
      m_pCurrentEntity->curstate.controller[2];
  m_pCurrentEntity->latched.prevcontroller[3] =
      m_pCurrentEntity->curstate.controller[3];
  ...
  if (pseqdesc->linearmovement[0] > 0) {
    m_pPlayerInfo->gaitframe +=
        (m_flGaitMovement / pseqdesc->linearmovement[0]) * pseqdesc->numframes;
  } else {
  ...
  m_pPlayerInfo->gaitframe =
      m_pPlayerInfo->gaitframe -
      (int)(m_pPlayerInfo->gaitframe / pseqdesc->numframes) *
          pseqdesc->numframes;
...
char *sNewWeaponPModels[] = {
    "models/p_9mmhandgun.mdl", "models/p_crowbar.mdl",
    "models/p_egon.mdl",       "models/p_glauncher.mdl",
    "models/p_grenade.mdl",    "models/p_knife.mdl",
    "models/p_medkit.mdl",     "models/p_mini.mdl",
    "models/p_nailgun.mdl",    "models/p_srpg.mdl",
    "models/p_shotgun.mdl",    "models/p_snailgun.mdl",
    "models/p_sniper.mdl",     "models/p_spanner.mdl",
    "models/p_umbrella.mdl",   "models/p_rpg.mdl",
    "models/p_spygun.mdl",     "models/p_smallshotgun.mdl"};

char *sOldWeaponPModels[] = {
    "models/p_9mmhandgun2.mdl", "models/p_crowbar2.mdl",
    "models/p_egon2.mdl",       "models/p_glauncher2.mdl",
    "models/p_grenade2.mdl",    "models/p_knife2.mdl",
    "models/p_medkit2.mdl",     "models/p_mini2.mdl",
    "models/p_nailgun2.mdl",    "models/p_rpg2.mdl",
    "models/p_shotgun2.mdl",    "models/p_snailgun2.mdl",
    "models/p_sniper2.mdl",     "models/p_spanner2.mdl",
    "models/p_umbrella.mdl",    "models/p_rpg2.mdl",
    "models/p_9mmhandgun2.mdl", "models/p_shotgun2.mdl"};
...
int CStudioModelRenderer::StudioDrawPlayer(int flags, entity_state_t *pplayer) {
  ...
  if (pplayer->fuser1) {
    ...
    // get the model pointer using a "corrected" model string based on
    // tfc_newmodels
    m_pRenderModel = gEngfuncs.CL_LoadModel(
        ReturnCorrectedModelString(iSwitchClass), &modelindex);
  }
  ...
  if (flags & STUDIO_RENDER) {
    ...
    if (pplayer->weaponmodel) {
      ...
      model_t *pweaponmodel =
          IEngineStudio.GetModelByIndex(pplayer->weaponmodel);
      ...
      if (strstr(m_pStudioHeader->name, "p_mini.mdl")) {
        if (g_flSpinUpTime[m_nPlayerIndex] &&
            g_flSpinUpTime[m_nPlayerIndex] > gEngfuncs.GetClientTime()) {
          float flmod = (g_flSpinUpTime[m_nPlayerIndex] -
                         (gEngfuncs.GetClientTime() + 3.5));
          flmod *= -30;
        ...
        else if (g_flSpinUpTime[m_nPlayerIndex] &&
                 g_flSpinUpTime[m_nPlayerIndex] <= gEngfuncs.GetClientTime())
          g_flSpinUpTime[m_nPlayerIndex] = 0.0;

        else if (g_flSpinDownTime[m_nPlayerIndex] &&
                 g_flSpinDownTime[m_nPlayerIndex] > gEngfuncs.GetClientTime() &&
                 !g_flSpinUpTime[m_nPlayerIndex]) {
          float flmod = (g_flSpinDownTime[m_nPlayerIndex] -
                         (gEngfuncs.GetClientTime() + 3.5));
          flmod *= -30;
        ...
        if (m_pCurrentEntity->curstate.sequence == 70 ||
            m_pCurrentEntity->curstate.sequence == 72) {
          if (g_flSpinUpTime[m_nPlayerIndex])
            g_flSpinUpTime[m_nPlayerIndex] = 0.0;
...
void CStudioModelRenderer::StudioCalcAttachments(void) {
  ...
  pattachment = (mstudioattachment_t *)((byte *)m_pStudioHeader +
                                        m_pStudioHeader->attachmentindex);
  for (i = 0; i < m_pStudioHeader->numattachments; i++) {
    VectorTransform(pattachment[i].org,
                    (*m_plighttransform)[pattachment[i].bone],
                    m_pCurrentEntity->attachment[i]);
  }
}
...
void CStudioModelRenderer::StudioRenderFinal_Software(void) {
  ...
  if (m_pCvarDrawEntities->value == 2) {
    ...
    for (i = 0; i < m_pStudioHeader->numbodyparts; i++) {
      IEngineStudio.StudioSetupModel(i, (void **)&m_pBodyPart,
                                     (void **)&m_pSubModel);
...
void CStudioModelRenderer::StudioRenderFinal_Hardware(void) {
  ...
  rendermode = IEngineStudio.GetForceFaceFlags()
                   ? kRenderTransAdd
                   : m_pCurrentEntity->curstate.rendermode;
